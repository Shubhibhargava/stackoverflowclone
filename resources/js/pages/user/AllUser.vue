<template>
    <div>
        <div class="container">
        <div class="row">
            <div class="col">
                <sidebar></sidebar>
            </div>
            <div class="col-10 maindiv">
                <div class="row headrow">
                    <div class="col">
                         <h1>Users</h1>
                    </div>
                </div>
                <div class="row inforow">
                    <div v-for="user in userinfo" v-bind:key="user.id" class="col-md-3">
                        <div class="row">
                        <div class="col-md-2">
                        <img  class="perimg" src="/images/permanent.png" >
                        </div>
                        
                         <div class="col-md-10 userdata">
                        <p class="nname">{{user.users.name}}</p>
                        <p class="ninfo">{{user.users.email}}</p>
                                    <p class="ninfo">{{user.tags}}</p>
                         </div>
                        </div>
                        </div>
                         <!-- <p><router-link :to="{ name: 'viewtag', params: { id: ta.id }}"><span class="tag">{{ta.name}}</span></router-link></p>
                         <p class="tagdes" v-if="ta.description.length<8">{{ta.description}}</p>
                         <p class="tagdes" v-else>{{ ta.description.substring(0,60)+"..." }}</p> -->
                        </div>
                    </div>
                    
                </div>
            </div>
        </div>
   
</template>
<script>
import sidebar from '../../components/SideBar.vue'
export default {
     data() {
      return {
        has_error: false,
        userinfo: null,
       
      }
    },
    components: {
        sidebar
    },
    mounted() {
      this.getTags()
    },
    methods: {
      getTags() {
        this.$http({
          url: `alluser`,
          method: 'GET'
        })
            .then((res) => {
              this.userinfo = res.data.userinfo
            }, () => {
              this.has_error = true
            })
      }
      
    }
}
</script>

<style scoped>

.headrow{
   
    padding: 20px;
    

}
.maindiv{
    min-height: 600px;
     border-left: 1px solid #D6D9DC;
}
.contentrow{
    padding: 10px;
  
}
.contentrow p{
    margin: 0 !important;
}
.tag{
   margin:4px;
    background:#E1ECF5;
    padding:0px 4px;
  
    border-radius: 3px;
  
    align-items: center;
    color: #3F78A0;
    font-size: 15px;
    cursor: pointer;
}
.tagdes{
    color: #9199A0;
    font-size: 13px;
}
.first{
    padding-top: 15px;
}
.perimg{
    height: 30px;
    width: 30px;
}
.userdata p{
   margin-bottom: 0px;
}
.nname{
    color: #4AA3FE;
    font-size: 18px;
}
.ninfo{
    color: #737C85
}
.inforow{
    margin-top: 1%;
}

</style>