      ...
      <div class="modal-wrapper">
          ...
          <div class="modal-body">
            <div class="row" v-if="status == 'success'">
               ...
               <div class="alert alert-success">
                  <p>User info updated successfully</p>
              </div>
                </div></div>
            <slot name="body">
              <div class="row">
       ...
       <form autocomplete="off" @submit.prevent="update" v-if="!success" method="post">
             <div class="form-group" v-bind:class="{ 'has-error': error && errors.name }">
                ...
                <input type="text" id="name" class="form-control" v-model="users.users.name" required>
                <span class="help-block" v-if="error && errors.name">{{ errors.name }}</span>
            </div>
            <div class="form-group" v-bind:class="{ 'has-error': error && errors.email }">
                <label for="email" class="lab">E-mail</label>
                <input type="email" id="email" class="form-control"  v-model="users.users.email" required>
    ...
    methods: {
      getUsers() {
       ...
       update(){
                 var app = this
                this.$http({
                     url: `updateuser/`+this.$route.params.id,
                    method: 'POST',
                    data: {
                       name:this.users.users.name,
                       email:this.users.users.email
